...
class ExperimentModel(Model):
    def build_graph(self):
        ...
        self._rnn_cell = tf.contrib.rnn.BasicLSTMCell(128)
    ...
    def rnn_initial_state_placeholder(self):
        return tf.contrib.rnn.LSTMStateTuple(tf.placeholder_with_default(self.rnn_zero_state.c,
                                                                         shape=[None, self._rnn_cell.state_size.c]),
                                             tf.placeholder_with_default(self.rnn_zero_state.h,
                                                                         shape=[None, self._rnn_cell.state_size.h]))
...
if __name__ == '__main__':
    import os
    import numpy as np
    from datasets.TinyShakespeareDataSet import TinyShakespeareDataSet
            ...
            if step % 1000 == 0:
                ...
                print(
                    'Step {}/{}: {}'.format(step, total_steps,
                                            repr(''.join(primer ) + '|' + ''.join(model.sample(primer=primer, sample_size=150)))))
